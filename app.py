...
def show_selected_chart(selected_chart,top_dict):
    ...
    if selected_chart == '词云图':
        ...
        c=(
            ...
            .set_global_opts(
                title_opts=opts.TitleOpts(title="词频"),
                xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(rotate=45)),
                yaxis_opts=opts.AxisOpts(type_="value", name="Y Axis"),
            )
        )
        streamlit_echarts.st_pyecharts(c)
...
def main():
    ...
    url = st.text_input("请输入一个URL",
                        "https://www.robotcontest.cn/datacenter/news/dynamicevents")
...
# 运行命令：streamlit run E:\studio\pythonProject\Python_爬虫\python实训\app.py